...
public class Login extends JFrame {
	...
	public Login() {
		...
		//	JOptionPane.showMessageDialog(null, textField_1.getText()," ", JOptionPane.INFORMATION_MESSAGE); //ovo je message box za provjeru
				String a="šef";
				if(textField.getText().equals(a) && passwordField.getText().equals(a)) //ispituje polja za unos usernamea i šifre
					{
				...
				String b="sluzbenik";
				if(textField.getText().equals(b) && passwordField.getText().equals(b)) //ispituje polja za unos usernamea i šifre
					{
					SluzbenikGui.Pocetni n =new SluzbenikGui.Pocetni();  //kreira novi početni gui za šefa
					n.setLocationRelativeTo(null);   // postavlja ga na sredinu
					n.setVisible(true);  // upali vidljivost
					n.setResizable(false);
					Login.this.dispose(); //zatvara ovu login formu
		...
		//ImageIcon _imageIcon= new ImageIcon(getClass().getResource("MicroOrg/resources/logo.jpg"));
